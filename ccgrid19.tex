...
The rest of this paper is organized as follows. Section~\ref{sec:orgc9b641f}
...
exploration strategies. Section~\ref{sec:org8f0cc11} discusses the Design
of Experiments, ANOVA, and linear regression methodology we used to develop our
approach. Section~\ref{sec:orgaa53d92} discusses the
implementation of our approach in detail. Section~\ref{sec:org50bcb89}
presents the results on the GPU Laplacian Kernel and on the SPAPT benchmark
suite. Section~\ref{sec:org89704da} discusses our conclusions and future work.
\section{Background}
\label{sec:orgc9b641f}
...
Section~\ref{sec:org50bcb89} were obtained in the domain of
...
Figure~\ref{fig:orgbd9e098} shows the contour of a search space defined
...
Section~\ref{sec:org50bcb89} can exceed 20 minutes, including the time
...
Figure~\ref{fig:orgbd9e098} are based on the Design of Experiments
...
Figure~\ref{fig:orgbd9e098}, a D-Optimal design using a linear model
...
as was shown in Figure~\ref{fig:orgbd9e098}.
...
Figure~\ref{fig:org0c91ace} presents an overview of our approach.

\begin{figure}[b]\vspace{-.5cm}
\centering
\includegraphics[width=.95\columnwidth]{./img/doe_anova_strategy.pdf}
\caption{\label{fig:org0c91ace}
...
The next four steps of an iteration, shown in Figure \ref{fig:org0c91ace},
...
\ref{tab:orgfda2dab}. Considering only factors and levels, the size of the
...
approach, described Section~\ref{sec:orgaa53d92}, except
...
Figure~\ref{fig:org6f14b62} presents histograms with the
...
kernels shown in Table~\ref{tab:orgca5a6b1} are implemented using the code
...
Figure~\ref{fig:org0c91ace}. Experiments were performed using
...
Figure~\ref{fig:org9865c84} presents the \emph{speedup} found by each
...
Figure~\ref{fig:orgd538c25} shows the search space exploration performed
by RS and DLMT. It uses the same color groups as
Figure~\ref{fig:org9865c84}, and shows the distribution of the
...
\caption{\label{fig:orgd538c25}
Histograms of explored search spaces, showing the real count of measured configurations. Kernels are grouped in the same way as in Figure~\ref{fig:org9865c84}. DLMT spent less measurements than RS in configurations with smaller speedups or with slowdowns, even for kernels in the orange group. DLMT also spent more time exploring configurations with larger speedups}
\end{figure*}

\section{Conclusion}
\label{sec:org89704da}
