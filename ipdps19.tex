...
The rest of this paper is organized as follows. Section~\ref{sec:org3ebff9b}
...
exploration strategies. Section~\ref{sec:orgd06c6bd} discusses the Design
of Experiments, ANOVA, and linear regression methodology we used to develop our
approach. Section~\ref{sec:orgba05db5} discusses the
implementation of our approach in detail. Section~\ref{sec:org5b8ed7b}
presents the results on the GPU Laplacian Kernel and on the SPAPT benchmark
suite. Section~\ref{sec:org4d2faf0} discusses our conclusions and future work.
\section{Background}
\label{sec:org3ebff9b}
...
Section~\ref{sec:org5b8ed7b} were obtained in the domain of
...
Figure~\ref{fig:org9ef16b4} shows the contour of a search space defined
...
Section~\ref{sec:org5b8ed7b} can exceed 20 minutes, including the time
...
Figure~\ref{fig:org9ef16b4} are based on the Design of Experiments
...
Figure~\ref{fig:org9ef16b4}, a D-Optimal design using a linear model
...
as was shown in Figure~\ref{fig:org9ef16b4}.
...
Figure~\ref{fig:orge54680c} presents an overview of our approach.

\begin{figure}[b]\vspace{-.5cm}
\centering
\includegraphics[width=.95\columnwidth]{./img/doe_anova_strategy.pdf}
\caption{\label{fig:orge54680c}
...
The next four steps of an iteration, shown in Figure \ref{fig:orge54680c},
...
\ref{tab:org6987dad}. Considering only factors and levels, the size of the
...
approach, described Section~\ref{sec:orgba05db5}, except
...
Figure~\ref{fig:org3c1dfaa} presents histograms with the
...
kernels shown in Table~\ref{tab:orgeafe009} are implemented using the code
...
Figure~\ref{fig:orge54680c}. Experiments were performed using
...
Figure~\ref{fig:org22172d9} presents the \emph{speedup} found by each
...
Figure~\ref{fig:org52a8ed7} shows the search space exploration performed
by RS and DLMT. It uses the same color groups as
Figure~\ref{fig:org22172d9}, and shows the distribution of the
...
we see in Figure~\ref{fig:org22172d9} that the \texttt{jacobi} kernel
...
\caption{\label{fig:org52a8ed7}
Histograms of explored search spaces, showing the real count of measured configurations. Kernels are grouped in the same way as in Figure~\ref{fig:org22172d9}. DLMT spent less measurements than RS in configurations with smaller speedups or with slowdowns, even for kernels in the orange group. DLMT also spent more time exploring configurations with larger speedups}
\end{figure*}

\section{Conclusion}
\label{sec:org4d2faf0}
