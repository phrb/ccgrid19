...
The rest of this paper is organized as follows. Section~\ref{sec:org15fc5af}
...
exploration strategies. Section~\ref{sec:orge1a2b5f} discusses the Design
of Experiments, ANOVA, and linear regression methodology we used to develop our
approach. Section~\ref{sec:org693fa96} discusses the
implementation of our approach in detail. Section~\ref{sec:org1e0cf82}
presents the results on the GPU Laplacian Kernel and on the SPAPT benchmark
suite. Section~\ref{sec:org3131562} discusses our conclusions and future work.
\section{Background}
\label{sec:org15fc5af}
...
Section~\ref{sec:org1e0cf82} were obtained in the domain of
...
Figure~\ref{fig:org798a03c} shows the contour of a search space defined
...
Section~\ref{sec:org1e0cf82} can exceed 20 minutes, including the time
...
Figure~\ref{fig:org798a03c} are based on the Design of Experiments
...
Figure~\ref{fig:org798a03c}, a D-Optimal design using a linear model
...
as was shown in Figure~\ref{fig:org798a03c}.
...
Figure~\ref{fig:orgc19e376} presents an overview of our approach.

\begin{figure}[b]\vspace{-.5cm}
\centering
\includegraphics[width=.95\columnwidth]{./img/doe_anova_strategy.pdf}
\caption{\label{fig:orgc19e376}
...
The next four steps of an iteration, shown in Figure \ref{fig:orgc19e376},
...
\ref{tab:orgb646dbb}. Considering only factors and levels, the size of the
...
approach, described Section~\ref{sec:org693fa96}, except
...
Figure~\ref{fig:orgc720db8} presents histograms with the
...
kernels shown in Table~\ref{tab:org3833d8c} are implemented using the code
...
Figure~\ref{fig:orgc19e376}. Experiments were performed using
...
Figure~\ref{fig:org095f37f} presents the \emph{speedup} found by each
...
Figure~\ref{fig:orgf8b3a6f} shows the search space exploration performed
by RS and DLMT. It uses the same color groups as
Figure~\ref{fig:org095f37f}, and shows the distribution of the
...
\caption{\label{fig:orgf8b3a6f}
Histograms of explored search spaces, showing the real count of measured configurations. Kernels are grouped in the same way as in Figure~\ref{fig:org095f37f}. DLMT spent less measurements than RS in configurations with smaller speedups or with slowdowns, even for kernels in the orange group. DLMT also spent more time exploring configurations with larger speedups}
\end{figure*}

\section{Conclusion}
\label{sec:org3131562}
